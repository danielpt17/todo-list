...
const TodoItem: React.FC<TodoItemProps> = ({ item, onDelete, onEdit }) => {
  return (
    <StyledTodoItem>
      ...
      <Field>{getPriority(item.priority)}</Field>
      <Field>{dayjs(item.dueDate).format('MMM D, YYYY')}</Field>
      <Field>{dayjs(item.creationTime).format('MMM D, YYYY h:mm')}</Field>
      <span>
        <DeleteButton title="delete" icon={<DeleteOutlined />} onClick={onDelete} aria-label="delete" />
        <EditButton title="edit" icon={<EditOutlined />} onClick={onEdit} aria-label="edit" />
      </span>
